  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.data.message === 'login successful') {
        await login(response.data.token, response.data.payload,'security');
        navigate('/security-profile', {
          state: {
            securityData: response.data.payload
          }
        });
      } else {
        ...
        <div className="col-md-8 offset-md-1">
          <div className="login-form-container">
            <div className="card p-4 p-md-5">
              <h4 className="text-center mb-4">Security Login</h4>
              {loginError && (
                <div className="alert alert-danger mb-3">{loginError}</div>
              )}
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className="mb-3">
                  <label htmlFor="username" className="form-label">Security ID</label>
                  <input
                    type="text"
                    id="username"
                    className={`form-control ${errors.username ? 'is-invalid' : ''}`}
                    {...register('username', { required: 'Security ID is required' })}
                  />
